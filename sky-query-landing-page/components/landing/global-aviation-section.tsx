  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>{Math.floor(Math.random() * 100) + 50} flights registered</span>
      </div>
    ...
    <div className="relative w-full h-full">
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100">
        ...
        {globePoints.map((point, i) => {
          const depthOpacity = Math.max(0.15, (point.z + 0.25) / 1.25) * 0.7;
          return (
            <circle
              key={i}
              cx={point.x * 100}
        ...
        {transformedRoutes.map((route, i) => {
          ...
          const fromX = route.from.x * 100;
          const fromY = route.from.y * 100;
          const toX = route.to.x * 100;
          const toY = route.to.y * 100;
      ...
      {transformedAirports
        ...
        .map((airport) => {
          ...
          const opacity = Math.max(0.5, (airport.z + 0.15) / 1.15);
          ...
          return (
            <motion.div
              ...
              transition={{ delay: 0.8 + Math.random() * 0.6 }}
              onMouseEnter={() => setHoveredAirport(airport.code)}
              onMouseLeave={() => setHoveredAirport(null)}
              onClick={() => onAirportClick(airport, airport.x * 100, airport.y * 100)}
            >
      ...
      {!shouldReduceMotion &&
        transformedRoutes.slice(0, 3).map((route, i) => {
          ...
          return (
            <AnimatedAircraft
              key={`aircraft-${i}`}
              fromX={route.from.x * 100}
              fromY={route.from.y * 100}
              toX={route.to.x * 100}
              toY={route.to.y * 100}
              duration={14 + i * 4}
              delay={i * 5}
